...
public:
	...
	// remaik:
	// 	1)  如果不设置, 默认不保留空格
	// 		这是为了兼容当前使用tinyxml方式加载的配置项,tinyxml默认是不保留空格的
	// 
	//  2)  仅对 GetData GetChildData GetAttrib GetChildAttrib 接口有效
...
public:
	CMarkup():m_bPreserveWhiteSpace(false) 
		{ x_InitMarkup(); SetDoc( NULL ); };
	CMarkup( MCD_CSTR szDoc ):m_bPreserveWhiteSpace(false)
		{ x_InitMarkup(); SetDoc( szDoc ); };
	CMarkup( int nFlags ):m_bPreserveWhiteSpace(false)
		{ x_InitMarkup(); SetDoc( NULL ); m_nDocFlags = nFlags; };
	CMarkup( const CMarkup& markup ):m_bPreserveWhiteSpace(false)
		{ x_InitMarkup(); *this = markup; };
	...
	MCD_STR GetData() 
	{ 
		MCD_STR str = x_GetData(m_iPos); 
		return m_bPreserveWhiteSpace ? str : Trim(str);
	};
	MCD_STR GetChildData() 
	{ 
		MCD_STR str = x_GetData(m_iPosChild); 
		return m_bPreserveWhiteSpace ? str : Trim(str);
	};
	MCD_STR GetElemContent() const { return x_GetElemContent(m_iPos); };
	MCD_STR GetAttrib( MCD_CSTR szAttrib ) const 
	{ 
		MCD_STR str = x_GetAttrib(m_iPos,szAttrib); 
		return m_bPreserveWhiteSpace ? str : Trim(str);
	};
	MCD_STR GetChildAttrib( MCD_CSTR szAttrib ) const 
	{ 
		MCD_STR str = x_GetAttrib(m_iPosChild,szAttrib); 
		return m_bPreserveWhiteSpace ? str : Trim(str);
	};
